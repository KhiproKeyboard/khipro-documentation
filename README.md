...
## এক নজরে ক্ষিপ্র কম্পোজিশনাল লেআউট

![ক্ষিপ্র লেআউট এক নজরে](https://raw.githubusercontent.com/KhiproTeam/khipro-at-a-glance/refs/heads/main/the_table_2.svg)

# ১. ক্ষিপ্র কনসেপ্টের কম্প্রিহেনসিভ বর্ণনা
<a id='1.0'></a>


## ১.১ পৃথায়ক

1. ক্ষিপ্র-তে সেমিকোলন `;` হলো পৃথায়ক।

2. পৃথায়ক ব্যবহার করে লেখাকে ছিন্নায়িত বা অংশায়িত করা যায়। এটা সফটওয়্যারকে ধরে নিতে বাধ্য করবে যে পৃথায়কের আগের আর পরের অংশদ্বয় বিচ্ছিন্ন। যেমন:  
`ami;i` = `আমিই`  
`jh` = `ঝ` &emsp; `j;h` = `জহ`  
`ae` = `অ্যা` &emsp; `a;e` = `আএ`  
`ii` = `ঈ` &emsp; `i;i` = `ইই`  
...
3. পৃথায়ককে [যুক্তবর্ণ গঠন রোধ]() করতেও ব্যবহার করা যায়। যেমন:  
`বুধবার` = `budh;bar`  
তবে যদি সম্ভাব্য যুক্তবর্ণ না থাকে তাহলে পৃথায়ক দেওয়া লাগবে না। যেমন:  
`লিখতে` = `likhte`  
4. একটা শব্দে যতবার ইচ্ছা পৃথায়ক ব্যবহার করা যাবে। এবং সেই পৃথায়কের পরে ক্ষিপ্র শব্দের শুরুর মতো আচরণ করবে।  

> [!NOTE]
> শব্দের পরে সেমিকোলন লিখতে হলে দুবার সেমিকোলন চাপতে হবে।  
> `ami;;` = `আমি;`

## মডিফায়ার

ক্ষিপ্রতে একাধিক মডিফায়ার key রয়েছে। একটি হলো `f` যাকে `⁌` রূপে দেখানো হয় লেখার প্রয়োজনে; এবং অপরটি **স্ল্যাশ**।

### মডিফায়ার হিসেবে `f` (⁌)
[স্বরবর্ণ ও কারচিহ্নের পারস্পরিক রূপান্তরে]() এবং [কিছু ব্যঞ্জনবর্ণ লিখতে]() মডিফায়ার ব্যবহার করা হয়।

### মডিফায়ার হিসেবে `/` স্ল্যাশ
মডিফায়ার হিসেবে `/` ব্যবহার করা হয় `ৎ` লেখার জন্য। যেমন:  
`ৎ` = `t`  

### যুক্তবর্ণ ভাঙতে স্ল্যাশের ব্যবহার
যুক্তবর্ণ ভাঙতে যখন স্ল্যাশের ব্যবহার করা হয় তখন সেটাকে আমরা [স্লাইসার]() বলি। সেটা কীভাবে কাজ করে সেটা [যুক্তবর্ণ সেকশনে]() আলোচনা করা হবে।

## কারচিহ্ন ও স্বরবর্ণ লেখা
### সাধারণ স্বরবর্ণের তালিকা
...
- সাধারণভাবে, শব্দের শুরুতে, এবং স্বরের পরে উপরের ম্যাপিংগুলো দিয়ে স্বরবর্ণ তৈরি হবে; এবং ব্যঞ্জনধ্বনির পরে লিখলে স্বরবর্ণ তৈরি না হয়ে কারচিহ্ন তৈরি হবে।   
উদাহরণ: `am` = `আম` &emsp; `nam` = `নাম` 
- [মডিফায়ার]() ব্যবহার করে স্বরবর্ণকে কারচিহ্ন বানানো, কিংবা কারচিহ্নকে স্বরবর্ণ বানানো যায়।  
উদাহরণ: `am` = `আম` &emsp; `afm` = `াম` &emsp; `nam` = `নাম` &emsp; `nafm` = `নআম`
- উপরের ফিচারটি কখন কাজে লাগবে? যখন শব্দের মাঝে ভুলে যাওয়া কারচিহ্ন বসানো লাগে, কিংবা শুধু কারচিহ্ন দেখানো লাগে, তখন।

> [!NOTE] অ বাদে অন্যান্য স্বরবর্ণকে কারচিহ্ন বানানো যাবে। অ-এর কোনো কারচিহ্ন নেই। তাই `of`= `ঽ` তথা [অবগ্রহ]() ম্যাপ করা হয়েছে।

### অতিরিক্ত স্বরবর্ণের তালিকা
| অ্যা / ্যা | ওয়া / োয়া | ওয়ে / োয়ে | ওয়্যা |
|:-:|:-:|:-:|:-:|
| `ae`  | `wa`  | `we`  | `wae`  |

- [সাধারণ স্বরবর্ণের নিয়মগুলো]() এগুলোর ক্ষেত্রেও কাজ করবে; অর্থাৎ, এগুলোকেও মডিফায়ার চেপে আন্তঃরুপান্তর করা যাবে।  
এই ৪টির মধ্যে ৩টির পূর্ণরূপের পাশাপাশি সংক্ষিপ্তরূপও রাখা হয়েছে।
- অ্যা -এর সংক্ষিপ্ত রূপ `্যা`। সাধারণ স্বরবর্ণের মতো কখনো `ae` এবং কখনো `aef` দিয়ে এর রূপ দুটি লেখা যাবে।
- ওয়া -এর সংক্ষিপ্ত রূপ `োয়া`। উদাহরণ: `swa` = `সোয়া` &emsp; `ওয়ান` = `wan`
- ওয়ে -এর সংক্ষিপ্ত রূপ `োয়ে`। উদাহরণ: `swetfar` = `সোয়েটার` &emsp; `wetf` = `ওয়েট`
- ওয়্যা -এর কোনো সংক্ষিপ্ত রূপ রাখা হয়নি। উদাহরণ: `harrdfwaer` = `হার্ডওয়্যার`
...
2. বিসর্গ লিখতে oo ব্যবহার করা হবে। যেমন:  
   দুঃখ \= duookh  
3. [অনুস্বার লেখার নিয়ম](#n-onshbr-ebn-ng-lkh) আলাদাভাবে আলোচিত হয়েছে।  
...
1. বাংলা ভাষায় প্রচলিত যেকোনো যুক্তবর্ণ গঠনের জন্য ক্ষিপ্রতে আলাদা করে হসন্ত ব্যবহার করার প্রয়োজন হবে না। সেসব যুক্তবর্ণ আপনিই গঠিত হবে। যেমন:  
   ...
   ([যুক্তবর্ণের পূর্ণাঙ্গ তালিকা](#jktbrnr-prnngg-tlk) নিচে সংযুক্ত আছে।)  
...
ব-ফলাকে [অন্যান্য যুক্তবর্ণের](#onjnj-jktbrn) মধ্যে রাখা হয়েছে। অর্থাৎ ব-ফলার জন্য আলাদা ম্যাপিং রাখা হয়নি। ব এবং ব-ফলা উভয়ের জন্য b ব্যবহার করতে হবে। যেমন:    
বল = bol    
ধ্বনি = dhbni   
...
5. সেমিকোলন যেহেতু পৃথায়ক, তাই শব্দের পরে সেমিকোলন লিখতে হলে দুবার সেমিকোলন দিতে হবে। অন্যথায় একবার দিলেই হবে। যেমন:  
   সহজ; \= সহজ;;  
6. স্বরের পরে স্ল্যাশ (/) দিলে চন্দ্রবিন্দু হবে, তাই স্বরের পরে স্ল্যাশ লিখতে দুবার স্ল্যাশ দিতে হবে। অন্যথায় একবার দিলেই হবে। যেমন:  
   থানা/উপজেলা \= thana//upojela  
   দেশ/শহর \= desh/shohor  
8. গুণ চিহ্ন ও ভাগ চিহ্ন দেওয়া:  
   1. গুণ চিহ্ন দিতে \+f লিখতে হবে। যেমন:  
      ২×২ \= 2+f2  
   2. ভাগ চিহ্ন দিতে \-f লিখতে হবে। যেমন:  
      ২÷২ \= 2-f2  
9. টাকা চিহ্ন দিতে $ দিতে হবে। যেমন:  
   ৳২ \= $2  
10. রুপি চিহ্ন দিতে $f দিতে হবে। যেমন:  
   ₹২ \= $f2  
11. ডলার চিহ্ন দিতে \$$ দিতে হবে। যেমন:  
    $২ \= $$2
...
1. ZWNJ (zero width non-joiner) দিয়ে সাধারণভাবে যুক্ত হয়ে যাওয়া গ্লিফকে বিযুক্ত করে লেখা হয়। যেমন: শ তে উ-কার দিলে বেশিরভাগ ফন্ট সেটাকে যুক্ত করে দিয়ে অস্বচ্ছ গ্লিফে লেখে (যেমন: শু)।  
   স্বচ্ছ আকারে [স্বরচিহ্ন লেখার জন্য ক্ষিপ্রতে শর্টকাট](#shbchch-rpr-krchhn-lkh-sh-sh) রাখা হয়েছে। যেমন:  
   রু \= ru 	কিন্তু, র‌ু \= ruff  
...
3. ZWJ (zero width joiner) কোথাও কোনো কম্বিনেশনে একাধিক স্থানে যুক্ত করা সম্ভব হলে কোন দুটি বর্ণকে যুক্ত করা হবে তা নির্দিষ্ট করে দিতে ব্যবহৃত হয়। যেমন:  
   ...
   র \+ zwj \+ ্ \+ য \= র‍্য 	\[ক্ষিপ্রতে এটা কেবল rz দিয়েই লেখা যায়। তবে চাইলে r\`\`fzf এভাবেও লেখা যাবে\]  
   উপরে ব্যবহৃত \`\`f হলো ZWJ-এর জন্য ক্ষিপ্র-র ম্যাপিং। [ক্ষিপ্র-র পূর্ণাঙ্গ ম্যাপিং সেকশনে](#bshsh-kch-jktbrn-ebn-bbdh) এর উল্লেখ রয়েছে। 
...
বিভিন্ন সিস্টেমে ক্ষিপ্র কিবোর্ড ইনস্টলেশনের পদ্ধতির লিংক [ক্ষিপ্র-র ওয়েবসাইটে](https://khipro.khiproteam.com/#installation) দেওয়া আছে।
...
<table border="0.5" cellspacing="0" cellpadding="4">
  ...
  <tr>
    <td>wi</td>     <td>ঐ<br>-ৈ</td>     <td>wik<br>kwi</td>     <td>ঐক<br>কৈ</td>
  </tr>
  <tr>
    <td>w</td>     <td>ও<br>-ো</td>     <td>wstad<br>kwn</td>     <td>ওস্তাদ<br>কোন</td>
  </tr>
  <tr>
    <td>wu</td>     <td>ঔ<br>-ৌ</td>     <td>wucitz<br>nwu</td>     <td>ঔচিত্য<br>নৌ</td>
  </tr>
  <tr>
...
<table border="0.5" cellspacing="0" cellpadding="4">
  ...
  <tr>
    <td>{ব্যঞ্জন}wuf</td>     <td>{ব্যঞ্জন}-োউ</td>     <td>bwuf</td>     <td>বোউ</td>
  </tr>
  <tr>
...
<table border="0.5" cellspacing="0" cellpadding="4">
  ...
  <tr>
    <td>ng</td>     <td>ঙ</td>     <td>rong<br>ranga<br>rangga</td>     <td>রঙ<br>রাঙা<br>রাঙ্গা</td>
  </tr>
  <tr>
...
<table border="0.5" cellspacing="0" cellpadding="4">
  ...
  <tr>
    <td>ggg</td>     <td>গ্‌গ</td>     <td>shigggir</td>     <td>শিগ্‌গির</td>
  </tr>
  <tr>
...
<table border="0.5" cellspacing="0" cellpadding="4">
  ...
  <tr>
    <td>oo</td>     <td>ঃ</td>     <td>বিসর্গ, duookh = দুঃখ</td>
  </tr>
  <tr>
    <td>x</td>     <td>ং</td>     <td>অনুস্বার, baxla = বাংলা</td>
  </tr>
  <tr>
  ...
  <tr>
    <td>off</td>     <td>ঽ</td>     <td>অবগ্রহ</td>
  </tr>
  <tr>
...
2. ক্ষিপ্র-র মূল রিপোজিটরি [khipro-m17n](https://github.com/rank-coder/khipro-m17n) সহ অন্যান্য কয়েকটি ল্যাংগুয়েজে ক্ষিপ্র-র যেসব মডিউল তৈরি করা হয়েছে সেগুলোর একটা তালিকা ওয়েবসাইটের হোম পেজে [ক্ষিপ্র রিপোজিটরি সমূহ](https://khipro.khiproteam.com/#repositories) সেকশনে দেওয়া হয়েছে।
...
> [!NOTE]
> টাচস্ক্রিনের জন্য উপরে উল্লিখিত ম্যাপিংগুলো বাদ দেওয়ার পাশাপাশি ডাবল দাঁড়ি এবং ঈশ্বর চিহ্নের ম্যাপিংয়ে পরিবর্তন করতে হবে। ম্যাপিং দুটি ডটের পরিবর্তে দাঁড়ি দিয়ে হবে। অর্থাৎ `।f` = `॥` এবং `।ff` = `৺`।
